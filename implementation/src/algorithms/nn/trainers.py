...
def train_sgd(
    ...
    print(
        f"{'Epoch':>5} | {'Train Loss':>10} | {'Train Acc':>9} | {'Test Loss':>9} | {'Test Acc':>8} | {'LR':>10} | {'Time':>6}"
    )
    print("-" * 80)
    ...
    for epoch in range(n_epochs):
        ...
        for inputs, labels in tqdm(
            train_loader, leave=False, desc=f"Epoch {epoch+1}/{n_epochs}"
        ):
            inputs, labels = inputs.to(device), labels.to(device)
        ...
        metrics.record_epoch(
            train_loss, train_acc, test_loss, test_acc, current_lr, epoch_time
        )

        print(
            f"{epoch+1:>5} | {train_loss:>10.4f} | {train_acc:>8.2f}% | {test_loss:>9.4f} | {test_acc:>7.2f}% | {current_lr:>10.6f} | {epoch_time:>5.1f}s"
        )
...
def train_sgda(
    ...
    print(
        f"{'Epoch':>5} | {'Train Loss':>10} | {'Train Acc':>9} | {'Test Loss':>9} | {'Test Acc':>8} | {'LR':>10} | {'Decays':>6} | {'Time':>6}"
    )
    print("-" * 95)
    ...
    for epoch in range(n_epochs):
        ...
        for inputs, labels in tqdm(
            ...
            if epoch >= warmup_epochs:
                is_satisfied = optimizer.check_armijo_and_update_lr(
                    loss_before, loss_after, inner_product
                )
                if not is_satisfied:
                    print(
                        "  Armijo condition not satisfied; reducing learning rate.",
                        loss_before,
                        loss_after,
                        inner_product,
                    )
        ...
        print(
            f"{epoch+1:>5} | {train_loss:>10.4f} | {train_acc:>8.2f}% | {test_loss:>9.4f} | {test_acc:>7.2f}% | {current_lr:>10.6f} | {decay_count:>6} | {epoch_time:>5.1f}s"
        )
