...
Trong mục này, ta trình bày thuật toán đề xuất và thiết lập sự hội tụ của nó. Dựa trên các bổ đề và mệnh đề đã nêu ở Mục \ref{sec:preliminaries}, ta đưa ra kết quả chính như sau.

% --- THUẬT TOÁN 1 ---
\noindent \textbf{Thuật toán 1 (Thuật toán Thích nghi Gradient Descent - GDA)}

\indent \textbf{Bước 0.} Chọn $x^{0} \in C$, $\lambda_{0} \in (0, +\infty)$, $\sigma, \kappa \in (0, 1)$. Đặt $k=0$.

\indent \textbf{Bước 1.} Cho $x^{k}$ và $\lambda_{k}$, tính toán $x^{k+1}$ và $\lambda_{k+1}$ như sau:
\begin{equation}
    x^{k+1} = P_{C}(x^{k} - \lambda_{k}\nabla f(x^{k})),
\end{equation}
Nếu $f(x^{k+1}) \le f(x^{k}) - \sigma \langle \nabla f(x^{k}), x^{k} - x^{k+1} \rangle$ thì đặt $\lambda_{k+1} := \lambda_{k}$, ngược lại đặt $\lambda_{k+1} := \kappa\lambda_{k}.$

\indent \textbf{Bước 2.} Cập nhật $k := k + 1$. Nếu $x^{k+1} = x^{k}$ thì DỪNG, ngược lại quay về Bước 1.
...
\begin{remark} \label{rem:1}
    ...
    \end{equation}
    điều này có nghĩa là $x^{k}$ là một điểm dừng của bài toán. Nếu thêm vào đó, $f$ là giả lồi, từ (\ref{eq:rem1_2}), nó suy ra rằng $f(z) \ge f(x^{k})$ với mọi $z \in C$, hay $x^{k}$ là một nghiệm của $OP(f, C)$.
\end{remark}
...
\begin{theorem} \label{thm:convergence}
    Giả sử rằng dãy $\{x^{k}\}$ được tạo ra bởi Thuật toán GDA. Khi đó, dãy $\{f(x^{k})\}$ là hội tụ và mỗi điểm giới hạn (nếu có) của dãy $\{x^{k}\}$ là một điểm dừng của bài toán.
...
\begin{proof}
    Áp dụng Mệnh đề \ref{prop:2}, ta có
    \begin{equation} \label{eq:proof_3}
        f(x^{k+1}) \le f(x^{k}) + \langle \nabla f(x^{k}), x^{k+1} - x^{k} \rangle + \frac{L}{2} \| x^{k+1} - x^{k} \|^{2}.
    \end{equation}
    Trong (\ref{eq:rem1_1}), lấy $z = x^{k} \in C$, ta đi đến
    ...
    \end{equation}
    Bây giờ ta khẳng định rằng $\{\lambda_{k}\}$ bị chặn dưới tách khỏi số 0, hay nói cách khác, kích thước bước nhảy thay đổi hữu hạn lần.
    
    Thật vậy, giả sử ngược lại rằng $\lambda_{k} \to 0$. Từ (\ref{eq:proof_5}), tồn tại $k_{0} \in \mathbb{N}$ thỏa mãn
    \[ f(x^{k+1}) \le f(x^{k}) - \sigma \langle \nabla f(x^{k}), x^{k} - x^{k+1} \rangle \quad \forall k \ge k_{0}. \]
    Theo cách xây dựng của $\lambda_{k}$, bất đẳng thức cuối cùng ngụ ý rằng $\lambda_{k} = \lambda_{k_{0}}$ với mọi $k \ge k_{0}$. Điều này là mâu thuẫn. Và do đó, tồn tại $k_{1} \in \mathbb{N}$ sao cho với mọi $k \ge k_{1}$, ta có $\lambda_{k} = \lambda_{k_{1}}$ và
    ...
    \begin{align} 
        \| x^{k+1} - z \|^{2} &= \| x^{k} - z \|^{2} - \| x^{k+1} - x^{k} \|^{2} + 2 \langle x^{k+1} - x^{k}, x^{k+1} - z \rangle \label{eq:proof_7} \\
        &\le \| x^{k} - z \|^{2} - \| x^{k+1} - x^{k} \|^{2} + 2 \lambda_{k} \langle \nabla f(x^{k}), z - x^{k+1} \rangle. \label{eq:proof_8}
    \end{align}
    Gọi $\bar{x}$ là một điểm giới hạn của $\{x^{k}\}$. Tồn tại một dãy con $\{x^{k_{i}}\} \subset \{x^{k}\}$ sao cho $\lim_{i \to \infty} x^{k_{i}} = \bar{x}$. Trong (\ref{eq:proof_8}), đặt $k = k_{i}$ và lấy giới hạn khi $i \to \infty$. Lưu ý rằng $\| x^{k_{i}} - x^{k+1} \| \to 0$, $\nabla f$ là liên tục, ta có
    \begin{equation}
    ...
    \[ U := \{ x \in C : f(x) \le f(x^{k}) \forall k \ge 0 \}. \]
    Lưu ý rằng $U$ chứa tập nghiệm của $OP(f, C)$, và do đó, là không rỗng. Lấy $\hat{x} \in U$. Vì $f(x^{k}) \ge f(\hat{x})$ với mọi $k \ge 0$, nó ngụ ý rằng
    ...
    \end{equation}
    Áp dụng Bổ đề \ref{lemma:1} (Xu) với $a_{k} = \| x^{k+1} - \hat{x} \|^{2}$, $b_{k} = 2\lambda_{k} \langle \nabla f(x^{k}), x^{k} - x^{k+1} \rangle$, ta suy ra rằng dãy $\{\| x^{k} - \hat{x} \|\}$ là hội tụ với mọi $\hat{x} \in U$. Vì dãy $\{x^{k}\}$ là bị chặn, tồn tại một dãy con $\{x^{k_{i}}\} \subset \{x^{k}\}$ sao cho $\lim_{i \to \infty} x^{k_{i}} = \bar{x} \in C$. Từ (\ref{eq:proof_6}), ta biết rằng dãy $\{f(x^{k})\}$ là không tăng và hội tụ. Điều này ngụ ý rằng $\lim_{k \to \infty} f(x^{k}) = f(\bar{x})$ và $f(\bar{x}) \le f(x^{k})$ với mọi $k \ge 0$. Điều này có nghĩa là $\bar{x} \in U$ và dãy $\{\| x^{k} - \bar{x} \|\}$ là hội tụ.
    
    Do đó,
    \[ \lim_{k \to \infty} \| x^{k} - \bar{x} \| = \lim_{i \to \infty} \| x^{k_{i}} - \bar{x} \| = 0. \]
    Lưu ý rằng mỗi điểm giới hạn của $\{x^{k}\}$ là một điểm dừng của bài toán. Khi đó, toàn bộ dãy $\{x^{k}\}$ hội tụ về $\bar{x}$ – một điểm dừng của bài toán. Hơn nữa, khi $f$ là giả lồi, điểm dừng này trở thành một nghiệm của $OP(f, C)$.
\end{proof}

% --- NHẬN XÉT 2 ---
\begin{remark}
    Trong Thuật toán GDA, ta có thể chọn $\lambda_{0} = \lambda$, với hằng số $\lambda \le 2(1-\sigma)/L$.
    Khi đó, ta có $(1-\sigma)/\lambda_{0} - L/2 \ge 0$. Kết hợp với (\ref{eq:proof_5}), điều này ngụ ý rằng điều kiện $f(x^{k+1}) \le f(x^{k}) - \sigma \langle \nabla f(x^{k}), x^{k} - x^{k+1} \rangle$ được thỏa mãn và bước nhảy $\lambda_{k} = \lambda$ cho mọi bước $k$.
    Do đó, Thuật toán GDA vẫn có thể áp dụng cho bước nhảy hằng số $\lambda \le 2(1-\sigma)/L$. Đối với bất kỳ $\lambda \in (0, 2/L)$, tồn tại $\sigma \in (0, 1)$ sao cho $\lambda \le 2(1-\sigma)/L$. Kết quả là, nếu giá trị của hằng số Lipschitz $L$ đã được biết trước, ta có thể chọn bước nhảy hằng số $\lambda \in (0, 2/L)$ như trong thuật toán gradient descent (GD) để giải các bài toán quy hoạch lồi.
...
% --- THUẬT TOÁN 2 (GD) ---
\noindent \textbf{Thuật toán 2 (Thuật toán Gradient Descent - GD)}

\indent \textbf{Bước 0.} Chọn $x^{0} \in C$, $\lambda \in (0, 2/L)$. Đặt $k = 0$.

\indent \textbf{Bước 1.} Cho $x^{k}$, tính toán $x^{k+1}$ như sau:
\begin{equation}
    x^{k+1} = P_{C}(x^{k} - \lambda \nabla f(x^{k})).
\end{equation}
...
\begin{proof}
    ...
    \begin{align} 
        ...
        &\le M \| \nabla f(x^{k}) \|,
    \end{align}
    trong đó $M := \sup \{ \| x^{k} - x^{*} \| : k \ge k_{1} \} < \infty$. Từ (\ref{eq:cor_11}) và (\ref{eq:cor_12}), ta đi đến
...
Để kết thúc phần này, ta trình bày một biến thể ngẫu nhiên của Thuật toán GDA để áp dụng trong học sâu quy mô lớn. Xem xét bài toán sau:
\begin{equation}
    \min_{x} \mathbb{E}[f_{\xi}(x)],
\end{equation}
trong đó $\xi$ là tham số ngẫu nhiên và hàm $f_{\xi}$ là trơn L (L-smooth). Ta đang tạo ra một gradient ngẫu nhiên $\nabla f_{\xi^{k}}(x^{k})$ bằng cách lấy mẫu $\xi^{k}$ tại mỗi lần lặp $k$.
...
\[ \min_{x} \mathbb{E}[f_{\xi}(x)], \]
trong đó $x$ đại diện cho các tham số của mô hình (như trọng số trong mạng nơ-ron), $\xi$ là một tham số ngẫu nhiên, và $f_{\xi}(x)$ là một hàm trơn L, ta đang đối phó với một kịch bản nơi hàm mục tiêu được định nghĩa là giá trị kỳ vọng của một hàm ngẫu nhiên $f_{\xi}(x)$ nào đó. Khung này là điển hình trong học máy, nơi $f_{\xi}(x)$ thường đại diện cho hàm mất mát được tính toán trên một tập con (batch) của dữ liệu huấn luyện, và $\xi$ đại diện cho sự ngẫu nhiên trong việc lựa chọn tập con này.
...
% --- THUẬT TOÁN 3 (SGDA) ---
\noindent \textbf{Thuật toán 3 (Thuật toán Thích nghi Gradient Descent Ngẫu nhiên - SGDA)}

\indent \textbf{Bước 0.} Chọn $x^{0} \in C$, $\lambda_{0} \in (0, +\infty)$, $\sigma, \kappa \in (0, 1)$. Đặt $k=0$.

\indent \textbf{Bước 1.} Lấy mẫu $\xi^{k}$ và tính toán $x^{k+1}$ và $\lambda_{k+1}$ như sau:
\begin{equation}
    x^{k+1} = P_{C}(x^{k} - \lambda_{k}\nabla f_{\xi^{k}}(x^{k})),
\end{equation}
Nếu $f_{\xi^{k}}(x^{k+1}) \le f_{\xi^{k}}(x^{k}) - \sigma \langle \nabla f_{\xi^{k}}(x^{k}), x^{k} - x^{k+1} \rangle$ thì đặt $\lambda_{k+1} := \lambda_{k}$, ngược lại đặt $\lambda_{k+1} := \kappa\lambda_{k}.$
