...
Trong mục này, ta trình bày thuật toán đề xuất và chứng minh sự hội tụ của nó. Dựa trên các mệnh đề và bổ đề đã nêu ở Mục \ref{sec:preliminaries}, ta đưa ra kết quả chính như sau.
...
\begin{remark} \label{rem:1}
    ...
    \end{equation}
    điều này có nghĩa là $x^{k}$ là một điểm dừng của bài toán. Bên cạnh đó, nếu $f$ là hàm giả lồi thì từ (\ref{eq:rem1_2}), ta suy ra $f(z) \ge f(x^{k})$ với mọi $z \in C$, hay nói cách khác $x^{k}$ là một nghiệm của bài toán $OP(f, C)$.
\end{remark}
...
\begin{theorem} \label{thm:convergence}
    Giả sử rằng dãy $\{x^{k}\}$ được tạo ra bởi Thuật toán GDA. Khi đó, dãy $\{f(x^{k})\}$ hội tụ và mỗi điểm tới hạn (nếu có) của dãy $\{x^{k}\}$ là một điểm dừng của bài toán. Hơn nữa,
    \begin{itemize}
...
\begin{proof}
    ...
    \end{align}
    Gọi $\bar{x}$ là một điểm tới hạn của $\{x^{k}\}$. Tồn tại một dãy con $\{x^{k_{i}}\} \subset \{x^{k}\}$ sao cho $\lim_{i \to \infty} x^{k_{i}} = \bar{x}$. Trong (\ref{eq:proof_8}), đặt $k = k_{i}$ và lấy giới hạn khi $i \to \infty$. Lưu ý rằng $\| x^{k} - x^{k+1} \| \to 0$, $\nabla f$ là liên tục, ta có
    \begin{equation}
    ...
    Lưu ý rằng $U$ chứa tập nghiệm của $OP(f, C)$, nên nó không rỗng. Lấy $\hat{x} \in U$. Vì $f(x^{k}) \ge f(\hat{x})$ với mọi $k \ge 0$, ta có
...
Để kết thúc phần này, chúng tôi trình bày một biến thể ngẫu nhiên của Thuật toán GDA để áp dụng trong học sâu quy mô lớn. Ta xét bài toán sau:
\begin{equation}
...
Thuật toán SGDA dưới đây bao gồm một mô tả chi tiết. Chúng tôi dành lại các kết quả lý thuyết chặt chẽ của Thuật toán SGDA cho các nghiên cứu sau này.
