...
Phương pháp được đề xuất, giống như thuật toán GD, có nhiều ứng dụng trong học máy. Trong báo cáo này, nhóm chúng tôi trình bày lại ba ứng dụng tiêu biểu được trình bày trong bài báo, bao gồm lựa chọn đặc trưng có giám sát (\textit{supervised feature selection}), hồi quy (\textit{regression}) và phân loại (\textit{classification}), để chứng minh độ chính xác và hiệu quả tính toán so với các phương pháp thay thế khác.
...
\tag{14}
\end{equation}
trong đó $w = (w_1, \dots, w_p)^T$ là vectơ điểm số đặc trưng cần xác định. Vì hàm mục tiêu của bài toán (\ref{bai toan : 14}) có dạng phân thức, trong đó tử là một hàm lồi và mẫu là một hàm tuyến tính dương, nên nó là giả lồi trên tập ràng buộc. Do đó, chúng ta có thể giải bài toán (\ref{bai toan : 14}) bằng Thuật toán GDA.

Trong thực nghiệm, chúng tôi triển khai các thuật toán với tập dữ liệu Parkinsons, bao gồm 23 đặc trưng và 197 mẫu, được tải xuống tại \url{https://archive.ics.uci.edu/ml/datasets/parkinsons}. Ma trận hệ số tương đồng $Q$ được xác định bởi $Q = \delta I_p + S$ (Wang và cộng sự (2021)\cite{wang2021neurodynamic}), trong đó  $S = (s_{ij})$ là ma trận $p \times p$ với:
...
Giá trị tối ưu xấp xỉ của bài toán (\ref{bai toan : 14}) là $f(w^*) = 0.153711$ với thời gian tính toán $T = 6.096260s$ cho thuật toán đề xuất, trong khi $f(w^*) = 0.154013$, $T = 11.030719$ cho Thuật toán RNN. So sánh với Thuật toán RNN, thuật toán của chúng tôi vượt trội hơn cả về độ chính xác và thời gian tính toán.
...
Chúng tôi so sánh các thuật toán huấn luyện bài toán hồi quy logistic bằng cách sử dụng các tập dữ liệu Mushrooms và W8a (xem Malitsky và Mishchenko 2020) \cite{malitsky2020adaptive}. Phương pháp GDA được so sánh với thuật toán GD có kích thước bước là $1/L$ và phương pháp tăng tốc Nesterov. Kết quả tính toán được hiển thị trong Hình \hyperref[hinh3]{3} và \hyperref[hinh4]{4} tương ứng. Các hình này cho thấy rằng Thuật toán GDA vượt trội hơn Thuật toán GD và phương pháp tăng tốc Nesterov về giá trị hàm mục tiêu trong các lần lặp. Đặc biệt, Hình \hyperref[hinh4]{4} thể hiện sự thay đổi của giá trị hàm mục tiêu theo các hệ số $\kappa$ khác nhau. Trong hình này, ký hiệu "GDA\_0.75" tương ứng với trường hợp $\kappa = 0.75$. Hình \hyperref[hinh5]{5} trình bày việc giảm kích thước bước từ kích thước bước ban đầu tương ứng với kết quả trong Hình \hyperref[hinh4]{4}.

\begin{figure}[h]
\centering
\begin{subfigure}[b]{0.45\textwidth}
   \centering
   \includegraphics[width = \linewidth]{fig3-1}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.45\textwidth}
   \centering
...
\end{subfigure}
\caption{Kết quả tính toán
cho hồi quy logistic với
...
\caption{Kết quả tính toán hồi quy logistic với tập dữ liệu W8a}
\label{fig:hinh4}
\end{figure}

\begin{figure}[H]
\includegraphics[width=0.8\linewidth]{fig5}
\caption{Các bước nhảy thay đổi cho mỗi lần lặp trong hồi quy logistic với tập dữ liệu W8a}
\label{fig:hinh5}
\end{figure}
...
Để huấn luyện mô hình mạng nơ-ron này, chúng tôi sử dụng biến thể ngẫu nhiên của phương pháp GDA (Thuật toán SGDA) để so sánh với các thuật toán Stochastic Gradient Descent (SGD). Các kết quả tính toán được hiển thị trong Hình \hyperref[hinh6]{6} và \hyperref[hinh7]{7} cho thấy rằng Thuật toán SGDA vượt trội hơn Thuật toán SGD về độ chính xác kiểm tra (testing accuracy) và độ mất mát huấn luyện (train loss) qua các lần lặp.
