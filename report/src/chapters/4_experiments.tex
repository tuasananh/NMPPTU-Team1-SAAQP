...
Các hàm mục tiêu và ràng buộc không lồi được minh họa trong Ví dụ 1 và 2 lấy từ \cite{liu2022one}. Ngoài ra, Ví dụ 2 phức tạp hơn Ví dụ 1 về dạng hàm. Việc thực hiện Ví dụ 3 với hàm mục tiêu lồi và số chiều $n$ thay đổi cho phép đánh giá phương pháp đề xuất so với Thuật toán GD. Ví dụ 4 được thực hiện với hàm mục tiêu giả lồi và một số giá trị $n$ để đánh giá phương pháp được các tác giả đề xuất so với Thuật toán RNN.
...
\textbf{Ví dụ 4} Để so sánh Thuật toán GDA với Thuật toán RNN trong \cite{liu2022one}, xét bài toán \ref{eq:OP} với hàm mục tiêu
\[
...
C := \{Ax = b,\ g(x) \le 0\},
\]
trong đó $x \in \mathbb{R}^n$, vector tham số $\varrho = (\varrho_1, \varrho_2, \dots, \varrho_n)^\top$ với $\varrho_i > 0$, ma trận $A = (a_1, a_2, \dots, a_n) \in \mathbb{R}^{1 \times n}$ với $a_i = 1$ khi $1 \le i \le n/2$ và $a_i = 3$ khi $n/2 < i \le n$, và số $b = 16$. Các ràng buộc bất đẳng thức là
\[
g_i(x) = x_{10\cdot(i-1)+1}^2 + x_{10\cdot(i-1)+2}^2 + \cdots + x_{10\cdot(i-1)+10}^2 - 20,
\]
với $i = 1, 2, \dots, n/10$. Bảng~\ref{tab:ex4} trình bày kết quả tính toán của Thuật toán GDA và RNN. Lưu ý rằng hàm $-\ln(-z)$ là đơn điệu tăng với $z \in \mathbb{R}$, $z < 0$. Do đó, để so sánh giá trị tối ưu xấp xỉ qua các vòng lặp, các tác giả tính $-\ln(-f(x^*))$ thay vì $f(x^*)$, với nghiệm tối ưu xấp xỉ $x^*$. Với mỗi $n$, bài báo \cite{thang2024saaqp} giải bài toán \ref{eq:OP} để tìm giá trị $-\ln(-f(x^*))$, số vòng lặp (\#Iter) để dừng thuật toán, và thời gian tính toán (Time) tính bằng giây. Kết quả tính toán cho thấy thuật toán đề xuất vượt trội hơn Thuật toán RNN trong các kịch bản thử nghiệm cả về giá trị tối ưu và thời gian tính toán, đặc biệt khi chiều lớn.
...
\begin{tabular}{l|lll|lll}
\hline
$n$ & \multicolumn{3}{c|}{Thuật toán GDA (đề xuất)} & \multicolumn{3}{c}{Thuật toán RNN} \\
